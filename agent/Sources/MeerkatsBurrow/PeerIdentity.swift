...
public struct PeerIdentity: Equatable, Hashable {
    ...
    /// 対の一覧を鍵で引くための値。**手元の中だけで使う。**
    ///
    /// ADR-0017 決定3 はこれを「ランデブーに預ける名刺を引く値」と決めていたが、
    /// ADR-0018 でその用途が無くなった。
    /// **名刺の場所は対ごとに共有秘密から導く**ので、この値は外に出ない。
    ///
    /// 出ない値になったので、**塩が無いことも、全員に対して同じであることも問題にならない。**
    /// 出していた頃は、一度招待コードを渡した相手が対を切ったあとも永久に名刺を引けた。
    ///
    /// ハッシュのままにしてあるのは、公開鍵をそのまま辞書の鍵にするより短く、
    /// 保存したときに読みやすいためである。**安全のためではない。**
